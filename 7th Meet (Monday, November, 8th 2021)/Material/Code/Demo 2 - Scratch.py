...
def translation_img(src_img, shift_distance, shape_of_output_img):
    h, w = src_img.shape[:2]
    x_distance = shift_distance[0]
    y_distance = shift_distance[1]
    ts_mat = np.array([[1, 0, x_distance], [0, 1, y_distance]])
    out_img = np.zeros(shape_of_output_img, dtype='u1')
...
res = translation_img(img, shift_distance, img.shape)
